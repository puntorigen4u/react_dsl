/**
* Remote Deploy: A class to help deploy remotely accesible react_dsl instances.
* @name 	remote
* @module 	remote
**/
...
export default class remote extends base_deploy {
    ...
    async deploy() {
        ...
        if ((await this._isLocalServerRunning())==false) {
            this.context.x_console.title({ title:'Deploying NextJS instance with remote access', color:'green' });
            await this.logo();
            ...
            this.context.x_console.title({ title:'Updating local running NextJS instance', color:'green' });
            await this.logo();
    ...
    async run() {
        ...
        try {
            ...
            await sleep(1000);
            await this.launchTerminal('npx',['--yes','localtunnel','--port',this.context.x_state.central_config.port,'--subdomain',this.context.x_state.central_config.apptitle.toLowerCase()],this.context.x_state.dirs.app);
