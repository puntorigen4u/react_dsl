      ...
      return _asyncToGenerator(function* () {
        ...
        var extractIcons = (content, icons) => {
          ...
          if (content.indexOf('{icon:') > -1) {
            var elements = extract("{icon:[name]}", new_);
            if (elements.name && !icons.includes(elements.name)) {
              icons.push(elements.name);
              new_ = new_.replace("{icon:".concat(elements.name, "}"), '');
              resp.content = new_;
              resp.icons = icons;
        ...
        Object.keys(auto).forEach(tag => {
          ...
          Object.keys(auto[tag].attributes).forEach(attr_ => {
            ...
            var attr__ = attr_.replace(/{icon:[^}]*}/g, '');
            var attr = auto[tag].attributes[attr_];
            /*
            ...
            if (attr_.indexOf('{icon:list}') != -1) {
              var type__ = attr_.indexOf('{icon:help}') != -1 ? 'event-attribute' : 'attribute';
              type__ = attr_.indexOf('{icon:idea}') != -1 ? 'view-attribute' : type__;
              if (!auto[attr__]) {
                auto[attr__] = {
                  text: attr__,
        ...
        var tags = Object.keys(auto).map(tag => {
          return {
            text: auto[tag].text && auto[tag].text != '' ? auto[tag].text : tag,
            //.replaceAll('*',''),
            //text: tag, //.replaceAll('*',''),
  ...
  class react_dsl extends concepto {
    ...
    autocompleteContentTemplate(record, render) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (record.extends_ && record.extends_ != '') {
          html += render.placeholders("Extends {icon:idea}<b>".concat(record.extends_, "</b><br/>"));
        }
    ...
    createSystemFiles(processedNodes) {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this18.writeFile(g('@styles/globals.css'), globalsCSS);
        ...
        for (var _thefile_num in processedNodes) {
          ...
          if (_thefile.file.split('.').slice(-1) == 'omit') ; else if (_thefile.file.includes('.group') == true) {
            yield* function* () {
              ...
              _this28.debug('processing group ' + _thefile.file + ' of files', _thefile);
              var cheerio = require('cheerio');
    ...
    _readAssets() {
      ...
      return _asyncToGenerator(function* () {
        var resp = {},
          path = require('path');
        _this32.debug('_readAssets');
        _this32.debug_time({
          id: '_readAssets'
        });
        var assets = yield _this32.dsl_parser.getNodes({
          text: 'assets',
          level: 2,
          icon: 'desktop_new',
          recurse: true
    ...
    _readCentralConfig() {
      var _this33 = this;
      return _asyncToGenerator(function* () {
        _this33.debug('_readCentralConfig');
        var central = yield _this33.dsl_parser.getNodes({
          level: 1,
          recurse: false
        });
        ...
        if (!resp[':cache']) _this33.x_config.cache = false; // disables cache when processing nodes (@todo)
        //@temp 23nov22
        /*
        if (resp['ui'] == 'mui') {
    ...
    _readConfig() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var resp = {
          ...
          config_node = {};
        var search = yield _this34.dsl_parser.getNodes({
          text: 'config',
          level: 2,
          icon: 'desktop_new',
          recurse: true
        ...
        if (search.length > 0) {
            ...
            if (key.text.toLowerCase() == 'meta') {
              for (var meta_child of key.nodes) {
                ...
                if (meta_child.text.toLowerCase() == 'keywords') {
                  ...
                  resp.meta.push({
                    hid: yield _this34.hash(meta_child.nodes[0].text),
                    name: 'keywords',
              ...
              resp = _objectSpread2(_objectSpread2({}, resp), _this34.configFromNode(resp, key));
              //
            }
          }
          ...
          var central2 = JSON.parse(JSON.stringify(_this34.x_state.central_config));
        ...
        if (!resp.name) {
          var path = require('path');
          var dsl_folder = path.dirname(path.resolve(_this34.x_flags.dsl));
          var parent_folder = path.resolve(dsl_folder, '../');
          var folder = dsl_folder.replace(parent_folder, '');
          resp.name = folder.replace('/', '').replace('\\', '') + '_' + path.basename(_this34.x_flags.dsl, '.dsl');
    ...
    getParentNodes() {
      var _arguments4 = arguments,
        _this35 = this;
      return _asyncToGenerator(function* () {
        var id = _arguments4.length > 0 && _arguments4[0] !== undefined ? _arguments4[0] : _this35.throwIfMissing('id');
        var exec = _arguments4.length > 1 && _arguments4[1] !== undefined ? _arguments4[1] : false;
        var parents = yield _this35.dsl_parser.getParentNodesIDs({
          id,
          array: true
        });
        var resp = [];
        for (var parent_id of parents) {
          var node = yield _this35.dsl_parser.getNode({
            id: parent_id,
            recurse: false
          });
          var command = yield _this35.findValidCommand({
            node,
            object: exec
          });
    ...
    hash(thing) {
      var _this36 = this;
      return _asyncToGenerator(function* () {
        var resp = yield _this36.dsl_parser.hash(thing);
