  ...
  **/
  ...
  var properties = {
  	...
  	"grid-column-start": {
  		...
  		description: "Specifies a grid item’s start position within the grid column by contributing a line, a span, or nothing (automatic) to its grid placement."
  	},
  	"grid-template-rows": {
  		values: [
  			"auto"
  	...
  	"grid-area": {
  		...
  		description: "A shorthand property for grid-row-start, grid-column-start, grid-row-end and grid-column-end, specifying a grid item’s size and location within the grid row by contributing a line, a span, or nothing (automatic) to its grid placement, thereby specifying the edges of its grid area."
  	},
  	"grid-column-gap": {
  		values: [
  		],
  ...
  class base_ui {
    ...
    sharedAutocomplete() {
      return _asyncToGenerator(function* () {
        ...
        var props = Object.keys(css.properties);
        for (var _i2 of props) {
          var prop = css.properties[_i2];
          shared['css-prop-' + _i2] = {
            type: 'css-property',
            text: _i2,
            ...
            level: [5, 6, 7, 8, 9],
            icons: ['list'],
            childrenTypes: ['css-' + _i2 + '-value'],
            attributes: {}
          };
          // generate AC items for each value of current prop
          for (var type_ of prop.values) {
            shared['css-' + _i2 + '-value-' + type_] = {
              type: 'css-' + _i2 + '-value',
              text: type_,
              hint: "Value for css property <b>".concat(_i2, "</b>."),
              level: [6, 7, 8, 9, 10],
              icons: [],
              childrenTypes: ['none'],
              attributes: {}
    ...
    defaultState() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this5.context.x_state.ui = _objectSpread2(_objectSpread2({}, _this5.context.x_state.ui), {
          ...
          'viewNPM': '',
          viewNPMSingle: false,
          //false -> import { view } from 'viewNPM'; false -> import view from 'viewNPM/view';
  ...
  class react_dsl extends concepto {
    ...
    onInit() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this3.x_state.npm['react-state-proxy'] = '^1.4.11';
        // axios
      ...
      return _asyncToGenerator(function* () {
        ...
        if (page) {
          ...
          if (Object.keys(page.variables) != '') {
            ...
            react.variables = "const $variables = stateProxy(".concat(_this12.jsDump(page.variables), ");\n");
            //react.script += `data() {\n`;
    ...
    processInternalTags(react, page) {
      ...
      return _asyncToGenerator(function* () {
        ...
        nodes = $("react_mounted").toArray();
        if (nodes.length > 0) _this13.debug('post-processing react_mounted tag');
    ...
    createSystemFiles(processedNodes) {
      ...
      return _asyncToGenerator(function* () {
        ...
        var appJSX = "import React, { useState, useEffect } from 'react';\n        import ReactDOM from \"react-dom\";\n        {appJSX.imports}\n\n        // import each main page\n        {appJSX.import_pages}\n\n        import \"./styles/globals.css\";\n\n        export const App = (props) => {\n        return (\n            {appJSX.open}\n            <div className=\"container\">\n            {appJSX.routes}\n            </div>\n            {appJSX.close}\n        )\n        };\n        ReactDOM.render(<App />, document.getElementById(\"app\"));\n        ";
          ...
          var toDisk = /*#__PURE__*/function () {
            var _ref3 = _asyncToGenerator(function* (thefile) {
              ...
              if (Object.keys(page.styles).length > 0) {
                ...
                var copy_ = _objectSpread2({}, page.styles);
                for (var _key4 in copy_) {
                  if (Array.isArray(copy_[_key4])) {
                    var sObj = {};
                    for (var i = 0; i < copy_[_key4].length; i++) {
                      var keys_ = Object.keys(copy_[_key4][i]);
                      for (var j = 0; j < keys_.length; j++) {
                        var key_ = keys_[j];
                        var value_ = copy_[_key4][i][key_];
                        sObj[key_] = value_;
                      }
    ...
    _readConfig() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (search.length > 0) {
          ...
          for (var _key5 in resp) {
            if (typeof resp[_key5] === 'object') {
              for (var key2 in resp[_key5]) {
                if (typeof resp[_key5][key2] === 'string' && resp[_key5][key2].indexOf('${') != -1 && resp[_key5][key2].indexOf('}') != -1) {
                  var to_eval = resp[_key5][key2];
