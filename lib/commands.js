module.exports = async function(context) {
    ...
    const parseInputOutput = async function(node,state) {
        ...
        if (tmp.var.includes('$')) {
            if (state.from_server) {
                ...
                tmp.var = tmp.var.replaceAll('$variables.', '$variables.')
                                .replaceAll('$vars.', '$variables.')
        ...
        if (tmp.original.includes('**') && node.icons.includes('bell')) {
            ...
            if (state.from_server) {
                ...
                tmp.original = tmp.original.replaceAll('$variables.', '$variables.')
                                            .replaceAll('$vars.', '$variables.')
                                            .replaceAll('$config.', 'this.$config.')
                                            .replaceAll('$store.', 'this.$store.state.');
    ...
    return {
        ...
        'def_main_style': {...null_template,
            ... {
                x_icons: 'desktop_new',
                x_level: '2',
                x_text_contains: 'style'
            }
        },
        ...
        'def_page': {
            ...
            func: async function(node, state) {
                ...
                if (node.icons.includes('bookmark')==true) context.x_state.pages[resp.state.current_page].shared = true;
        ...
        'def_page_styles': {
            ...
            hint: 'Custom page/component styles node (virtual)',
            func: async function(node, state) {
                let resp = context.reply_template({
                    state
                });
                //let params = aliases2params('def_page_styles', node);
                //resp.open = context.tagParams('page_styles', params, false);
                //resp.close = '</page_styles>';
                resp.state.from_styles=true;
                return resp;
            }
        },
        'def_page_styles_class': {
            ...
            hint: 'Custom page/component class',
            func: async function(node, state) {
                let resp = context.reply_template({
                    state,
                    hasChildren:false
                });
                const classes = context.x_state.pages[resp.state.current_page].styles;
                let name = node.text.trim();
                // if name is not a standard html tag, add a dot prefix
                if (name[0]!='.' && ['a','abbr','address','area','article','aside','audio','b','base','bdi','bdo','blockquote','body','br','button','canvas','caption','cite','code','col','colgroup','data','datalist','dd','del','details','dfn','dialog','div','dl','dt','em','embed','fieldset','figcaption','figure','footer','form','h1','h2','h3','h4','h5','h6','head','header','hgroup','hr','html','i','iframe','img','input','ins','kbd','label','legend','li','link','main','map','mark','meta','meter','nav','noscript','object','ol','optgroup','option','output','p','param','picture','pre','progress','q','rb','rp','rt','rtc','ruby','s','samp','script','section','select','slot','small','source','span','strong','style','sub','summary','sup','table','tbody','td','template','textarea','tfoot','th','thead','time','title','tr','track','u','ul','var','video','wbr'].indexOf(name)==-1) {
                    name = '.'+name;
                }
                let current = {...node.attributes};
                // recurse children to update 'current' object
                if (node.nodes_raw.length > 0) {
                    let children = await node.getNodes()
                    for (let child of children) {
                        let key = child.text.trim();
                        if (child.icons.includes('button_cancel')) {
                            // omit child if it's canceled (this shouldn't be necessary)
                        } else if (child.icons.includes('list')) {
                            // child is an attribute
                            let value = '';
                            if (child.link!='' && child.link.indexOf('ID_')!=-1) {
                                // get value from link
                                const childLinkNode = await context.dsl_parser.getNode({ id:child.link, nodes_raw:false, recurse:true });
                                value = childLinkNode.text;
                            } else if (child.bgcolor!='') {
                                // get value from node.bgcolor
                                value = child.bgcolor;
                            } else if (child.nodes_raw.length==1) {
                                // child contains exactly 1 child, assign its node text as value
                                let children2 = await child.getNodes();
                                value = children2[0].text;
                            }
                            current[child.text.trim()] = value;
                        } else if (child.text=='&' && Object.keys(child.attributes).length>0) {
                            // child has no icons, but has attributes and a text of & (means apply attrs to all of its children)
                            current[child.text.trim()] = {};
                            let children2 = await child.getNodes();
                            for (let child2 of children2) {
                                current[child2.text.trim()] = {...child.attributes};
                            }
                            delete current['&'];
                        } else if (Object.keys(child.attributes).length>0) {
                            // child has no icons, but has attributes (value is an object of the child)
                            // test if child has children and are contain 'list' icon (attribs)
                            // @todo maybe here we should refactor to call ourselfs recursively
                            let selfs = await context.x_commands['def_page_styles_class'].func(child, { ...state, ...{ inner_call:true }});
                            current[child.text.trim()] = {...selfs.state.classes};
                            //context.debug('selfs state',selfs.state);
                        }
                    }
                }
                // assign
                if (name!='.&') {
                    let arr = [];
                    if (classes[name] && !Array.isArray(classes[name])) {
                        // transform object to array && push
                        // @todo rethink this... I believe its wrong 11-12-2022 (@recheck)
                        arr.push({...classes[name]});
                        //arr.push(current); //23dic22 - this works best commented out
                        //context.debug('array of classes',arr);
                        classes[name] = arr;
                    } else if (classes[name] && Array.isArray(classes[name])) {
                        arr.push(current);
                        classes[name] = arr;
                    } else {
                ...
                // reassign classes to current page styles obj 
                context.x_state.pages[resp.state.current_page].styles = classes;
                //context.debug('classes of page',classes);
                resp.state.classes = current;
                return resp;
            }
        },

        'def_page_styles_class_omit': {
            x_level: '4,5,6,7,8,9,10',
            x_all_hasparent: 'def_page_styles_class',
            hint: 'Field for custom page/component class (omit)',
            func: async function(node, state) {
                let resp = context.reply_template({
                    state,
                    hasChildren:false
        ...
        'def_componente': {
            ...
            func: async function(node, state) {
                ...
                /* @deprecated : test if we should support bit.dev within react (was from vue) */
        ...
        'def_view': {
            ...
            autocomplete: await (async()=>{
                ...
                let ac = {};
                let icons = `AboutUsPage,AcceptTerms,Account,ActiveSupport,ADayAtThePark,
                AddFiles,Address,AddToCart,AddUser,Agreement,Airport,AirSupport,Alert,AlienScience,
                Analysis,Android,AppInstallation,Appreciation,ArtificialIntelligence,ArtLover,Astronaut,
                AtWork,Autumn,Baby,BackInTheDay,BackToSchool,Basketball,Beach,BeerCelebration,BeforeDawn,
                BeTheHero,Bibliophile,BirthdayCake,Bitcoin,BitcoinP2P,BlankCanvas,Blogging,BlogPost,
                BookLover,Brainstorming,Broadcast,Browser,BrowserStats,Building,BuildingBlocks,BusinessDeal,
                Businessman,BusinessPlan,Businesswoman,BusStop,ByMyCar,Calculator,Calendar,Campfire,Camping,
                CautiousDog,Celebration,Charts,Chat,Chatting,Checklist,Chef,Chiling,Choose,CityDriver,CloudHosting,
                ...
                EasterEggHunt,EatingTogether,ElectricCar,EmailCapture,Emails,Empty,Envelope,Ether,Ethereum,Events,Everywhere,
                Experts,Exploring,
                FallsComing,Fans,FastLoading,Fatherhood,FeelingBlue,FileSearching,FilingSystem,Filter,Finance,Firmware,Fishing,
                Floating,Focus,FollowMeDrone,Forever,ForgotPassword,ForSale,Freelancer,Friendship,
                Gaming,Gardening,Gdpr,GettingCoffee,Gift,GirlsJustWannaHaveFun,Goal,GoldenGateBridge,GoodDoggy,Grades,Graduation,
                GrandSlam,GroupSelfie,Growing,
                HangOut,HappyBirthday,HealthyHabit,Hello,HighFive,Hiring,HomeRun,Houses,HouseSearching,
                ImageFolder,Images,ImageUpload,InboxCleanup,Influencer,InLove,InstantSupport,
                Sync,
                InternetOnTheGo,InTheOffice,InThePool,Invite,
                JavascriptFrameworks,JobHunt,Jogging,Lighthouse,LiveCollaboration,Loading,Login,Lost,LoveIsInTheAir,
                Mail,Mailbox,MailSent,Maintenance,MakeItRain,MakeupArtist,MapDark,MapLight,Marilyn,MayTheForce,Meditation,
                Meeting,Messages,Messaging,Messenger,Mindfulness,MissionImpossible,Mobile,MobileApps,MobileBrowsers,MobilePayments,
                ModernWoman,MomentToRemember,MoreMusic,MovieNight,Music,
                Navigation,Nerd,NewMessage,News,Newsletter,NoData,Notebook,Notes,
                OffRoad,OldDay,Onboarding,Online,OnlineShopping,OnlineWorld,OnTheOffice,OnTheWay,Opened,OrderConfirmed,OrdinaryDay,
                OrganizePhotos,OrganizeResume,OrganizingProjects,OuterSpace,PassingBy,PedestrianCrossing,PenTool,PeopleSearch,
                Personalization,Photo,Photocopy,Photos,PhotoSharing,PieChart,Pilates,PlainCreditCard,PlayfulCat,Podcast,Portfolio,
                Post,Posts,Powerful,Preferences,Presentation,PressPlay,Processing,ProductHunt,ProductTeardown,ProductTour,ProfilePic,
                Programming,QaEngineers,Queue,ReadingList,RealTimeSync,Refreshing,Relaxation,RelaxingAtHome,Report,Responsive,Resume,
                ResumeFolder,RideABicycle,Rising,RomanticGetaway,RunningWild,
                Safe,Science,Search,SearchEngines,SecureData,SecureServer,SecurityOn,Segment,Segmentation,Select,Selfie,SelfieTime,
                Server,ServerStatus,Setup,SiteStats,SleepAnalysis,Sleeping,SmileyFace,Snowman,SocialGrowth,SocialIdeas,SocialMedia,
                SocialNetworking,SocialStrategy,SocialTree,Specs,Spreadsheets,Starman,Startled,StartupLife,StaticAssets,Statistics,StayingIn,
                StreetFood,Studying,SuccessfulPurchase,SunnyDay,Superhero,Surfer,SwipeProfiles,Sync,Tabs,Taken,TakingNotes,Target,Tasting,Team,
                TeamSpirit,Thoughts,ThroughTheDesert,TimeManagement,ToDoList,Together,ToTheStars,TrackAndField,TransferFiles,Trip,Tweetstorm,
            ...
            func: async function(node, state) {
                ...
                if (context.x_state.ui.textSimpleIfParentView && context.x_state.ui.textSimpleIfParentView.includes(node.text)) {
        ...
        'def_textonly': {
            ...
            func: async function(node, state) {
                ...
                let text = base_text.replaceAll('$variables.', '$variables.')
                .replaceAll('$vars.', '$variables.')
                //.replaceAll('$params.', '')
                .replaceAll('$store.', '$store.state.');
                if (text == '') text = '&nbsp;';
                ...
                if ((await context.hasParentID(node.id, 'def_variables'))==true) {
                    return {...resp,...{ valid:false }};
        ...
        'def_event_init': {
            x_icons: 'help',
            x_level: '3,4',
            x_text_contains: 'init',
            x_or_isparent: 'def_page,def_componente',
            hint: 'Special virtual event init; runs its code before the component/page is rendered',
            func: async function(node, state) {
                let resp = context.reply_template({
                    state,
                    hasChildren: true
                ...
                if (node.nodes_raw.length==0) return resp;
                let params = {};
                resp.open = context.tagParams('react_init', {}, false)+'<!--';
                if (node.text_note != '') resp.open += `/*${node.text_note.cleanLines()}*/\n`;
                resp.close = '--></react_init>';
                resp.state.from_script=true;
                return resp;
            }
        },
        ...
        'def_literal_js': {
            ...
            func: async function(node, state) {
                ...
                if (node.text.includes('$variables.') && node.text.right(2)=='()') {
                    ...
                    tmp.text = tmp.text .replaceAll('$variables.','$variables.')
                                        .replaceAll('$vars.','$variables.');
                }
                //scrollTo plugin?
